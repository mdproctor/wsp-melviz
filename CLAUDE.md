...
- `@casehubio/pages-ui-components` — Standalone Lit web components: `PagesNumberInput`, `PagesDatePicker`, `PagesColorPicker`, `PagesSliderInput`, `PagesTagsInput`, `PagesMultiSelect`, `PagesTextArea`, `PagesCheckbox`, `PagesStatusBadge` (status pill with registry SPI), `PagesSplitWorkbench` (draggable split-pane layout), `PagesEventTrail` (timestamped event display with filtering). Styled with design tokens. Depends on `lit`.
- `@casehubio/pages-diagram-core` — Diagram editing infrastructure: `DiagramBaseMixin` (YAML load/save, undo/redo, selection, ELK layout pipeline, palette/property panel wiring), `PagesDiagramToolbar`, `exportDiagram` (SVG/PNG), `GitHubBackend` (GitHub Contents API persistence), `yamlSetField`/`yamlDeleteField` (CST-preserving YAML primitives), `registerPropertySchema`/`getPropertySchema`, `PagesPromptEditor`, `PagesJsonViewer`. Depends on `graph-core`, `graph-renderer`, `pages-property-palette`, `pages-diagram-palette`, `lit`, `yaml`, `html-to-image`.
