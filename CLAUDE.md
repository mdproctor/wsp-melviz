...
- `@casehubio/graph-work-registry` — Marketplace work stencil discovery: `WorkStencil` types, YAML descriptor parser, `WorkStencilRegistry` (URL-based fetch/parse/validate/cache), `CategoryIndex` (hierarchical palette-ready query API). Consumed by graph-renderer's default work stencil renderer.
