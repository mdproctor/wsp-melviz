...
- `@casehubio/pages-data-table` — Data table Web Component: single CSS Grid body, virtual scroll, sorting, filtering, column visibility (`hiddenColumns`), multi-mode selection, tree rows, row-detail expansion, cell spanning (`mergeRows`, `cellSpan`), CSV export, ARIA grid, keyboard navigation, native `groupBy` (interleaved group headers). Depends on `lit`. Migrated from blocks-ui in blocks-ui#48. Will rename to `pages-table` after TypedDataSet integration (blocks-ui#49).
