...
**#429 — Dock-Workbench Lit Extraction (P3):** The builder shell has a bespoke hardcoded dock layout. The codebase also has a runtime dock-workbench pipeline doing the same job. Extract the shared logic into a reusable `<pages-dock-workbench>` Lit component.
...
`beginTransaction()` saves the current `toString()` as the undo snapshot and sets an internal `_inTransaction` flag. All mutations during the transaction skip `_pushUndoInternal()`. `commitTransaction()` clears the flag. The pre-transaction snapshot is already on the undo stack — undo restores to the pre-compound state in one step.

Nested transactions are not needed — compound operations do not compose into deeper compounds. Every compound operation wraps in try/finally:
...
For container types (tabs, accordion, etc.): uses the descriptor registry to determine the correct slot structure. The wrapped component becomes the first entry with a default slot name ("Tab 1", "Section 1").

```typescript
replaceWith(type: string): ComponentNode
```
Compound (transaction-wrapped): create a new component of the target type at the same position, migrate common properties by key match (properties present in both source and target schemas transfer; source-only properties are dropped; target-only properties get schema defaults from `componentSchemaRegistry`), remove the original. Returns the new component node.
...
```
Page-level restructuring (transaction-wrapped): remove the specified components from their current positions, create a new row with a single full-width column, insert the components into that column, place the row at the position of the first removed component. If the page is in `flat` mode, automatically transitions to `rows` mode.
...
**Shared filtering logic:** A `PaletteFilter` utility is extracted from `builder-palette.ts`. Both the dock palette and the inline popover consume it. The filter takes a `PickerContext` and the full catalog, returns entries with relevance scores (promoted / normal / hidden / needs-prereq).
...
| `Ctrl+P` | Open palette popover at selected node |
...
| On a named-record container (e.g., tabs node itself) | Prompt for slot name, then `moveToSlot()` — create new slot |
...
A `PreviewDataRegistry` maps component types to `PreviewDataStrategy` implementations. Only data-consuming types register strategies (~20 of 56 catalog entries). Layout, content, form, and workbench types render with their default props — no strategy needed.
...
**ChartDataStrategy** — `bar-chart`, `line-chart`, `pie-chart`, `area-chart`, `radar-chart`, `scatter-chart`, `heatmap-chart`

Reads `lookup.uuid` from props → finds the referenced `DatasetNode` → reads column definitions → generates 5-8 rows with typed values. String columns get region/product/category names. Number columns get plausible values in the 1,000-100,000 range. Date columns get dates spanning the last 12 months.

**TableDataStrategy** — `data-table`, `grid-table`

Same dataset lookup. Generates 10-15 rows with varied types per column. Includes edge cases: one null value, one long string, one negative number — so the preview reveals how the table handles real data.

**MetricDataStrategy** — `metric`, `big-number`, `gauge`, `sparkline`

Generates a single numeric value (e.g., 42,350) with an optional 12-point trend array for sparklines. The value is derived from the metric's `lookup` column type if available.

**FilterDataStrategy** — `filter-chips`, `scope-selector`, `select-filter`

Reads the referenced column → generates 4-6 distinct categorical values. The filter renders with real-looking options instead of empty dropdowns.

**ListDataStrategy** — `list-pane`, `notification-inbox`

Generates 3-5 items with title, description, and timestamp fields. Timestamps span the last 48 hours.
...
The builder shell already has `renderPreview` callback and `_refreshPreview()`. The integration:

1. On document change, the builder constructs a `PreviewDataProvider` wrapping the strategy registry
2. For each component with a registered strategy, the provider pre-generates `DatasetSnapshot` entries
3. The provider is passed to `loadSite()` as a data source override
4. The runtime's `DataSourceController` checks for the provider before making HTTP requests — if a snapshot exists for the requested UUID, it returns the snapshot immediately
5. The existing preview-click synchronization (via `data-component-type` attributes) continues working — the components render normally, just with synthetic data

Re-generation triggers:
- Component props change (different dataset, different columns) → strategy regenerates
...
Three layers, refactored in order:

**Layer 1: `<pages-dock-workbench>` Lit component** — in `pages-primitives` (no new package, per workbench-integration-pattern protocol). Extracts layout logic from `ZoneLayoutEngine`: zone sizing, resize handles, panel collapse/expand, toggle bar, localStorage persistence.

**Layer 2: Runtime bridge** — `component-desugar.ts` still parses `dock-workbench` YAML into `DockWorkbenchConfig`. The runtime renders a `<pages-dock-workbench>` element with slotted content instead of building a raw `Component` tree of divs. Existing dock-workbench YAML continues working unchanged.

**Layer 3: Builder shell migration** — `builder-shell.ts` replaces its bespoke dock (~200 lines of layout CSS, resize event handlers, toggle bar) with `<pages-dock-workbench>`.
...
packages/
  ...
  pages-builder/src/                 # UI interactions + preview data
    ...
    data/
      ...
      preview-data-provider.ts       # NEW — runtime data source override
      strategies/
        chart-strategy.ts            # NEW
        table-strategy.ts            # NEW
        metric-strategy.ts           # NEW
        filter-strategy.ts           # NEW
        list-strategy.ts             # NEW
    shell/
      builder-shell.ts               # Migration: replace bespoke dock with <pages-dock-workbench>

  pages-primitives/src/              # Shared components
    context-menu/
...
- Transaction API: begin/commit, undo after compound, error during transaction rolls back cleanly
...
- Keyboard shortcuts: Delete removes, Ctrl+D duplicates, Ctrl+P opens picker
...
**Dock workbench tests** (`pages-primitives`):
...
- `packages/pages-runtime/src/site.ts` — runtime dock-workbench detection and ZoneLayoutEngine
...
- `docs/specs/issue-285-dock-workbench/` — original dock-workbench design spec
